...
class Persons(Dataset):
  def __init__(self, root, subset, h, w, means, stds, crop_h=None, crop_w=None):
    ...
    #Anpassungen um direkt die Datensets unserer Vorgänger zu nutzen
    #mask: Labeldatem (Original: img)
    #rgb: Rohbilder (Original: lbl)
    #Ordnerpfade: <ROOT>/rgb oder mask/<SUBSET> (train oder test)
    #Originaler parser: (root, subset, img/lbl)
    self.images_root = os.path.join(root, "rgb", subset)
    self.labels_root = os.path.join(root, "mask", subset)
    ...
    # Werte aus cityscapes/parser.py Zeilen 118 - 121 übernommen
    ...
    # Übernommen aus cityscapes/parser.py
  ...
  def __init__(self, img_prop, img_means, img_stds, classes, train, location=None, batch_size=None, crop_prop=None, workers=2):
    ...
    if self.train:
      ...
      self.validloader = torch.utils.data.DataLoader(self.valid_dataset,
                                                     ...
                                                     pin_memory=True,
                                                     drop_last=True)
      #Wir ignorieren ein leeres Validation / Test Datenset bis wir eines haben
